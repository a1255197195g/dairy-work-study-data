...
## dart中的数据类型
 ...
 首先：dart中所有类型（包括基础类型），都是类。也就是说一切皆对象。<font color='red'> dart中所有类型，系统默认初始化为null,  包括int，double类型的变量。</font>
...
```
#dart中定义私有属性或者方法，并没有关键字去修饰。而是使用(_)来表示私有属性或者私有方法。
#在同一个文件中，（_）标识的私有属性或者方法，还是能被同文件的类外部访问到。
#但在不同文件中，（_）标识的私有属性或者方法，不能被不同文件所访问到。
...
<font color='red'>在dart中，建议成员属性，尽可能定义成私有属性, 用get，set去操作属性。如下：</font>
...
## 使用 static 定义类方法(静态方法)，类属性(静态属性)

- dart中的static只能修饰类中的属性和方法，不能修改全局或者局部的变量或者对象。
- static修饰的 "静态属性" 是在程序运行之初就创建的，如果有赋值操作，则同时进行初始化。所以类的静态属性，静态方法是程序整个生命周期都存在的。

* 类中的 "静态方法" 方法体内，不能调用类实例的成员属性，成员方法；因为"静态方法"调用时，可能还没有类实例存在。
* dart中的 "静态方法","静态属性" 只能通过类名来调用，不允许通过类实例来调用（包括在类内部，通过this去调用）。

```dart
void main() {
  //  static int a;  报错; 不允许static修饰全局或者局部变量，方法。
  int a;
  print(a);

  Person.prinntCount();
  Person person = new Person();
  // person.prinntCount(); 报错，不允许使用 类实例 去访问静态方法或者静态属性
  Person.prinntCount();
}

class Person {
  //使用static放在类型，变量之前进行修饰，表示这个属性是静态属性。
  //dart中所有类型，系统默认初始化为null。
  static int _count = 0;
  Person() {
    _count++;
    //  this.prinntCount();  通过this关键字去调用 静态方法 ，报错。
    //  this._count++;       通过this关键字去调用 静态属性 ，报错。
  }

  //使用static关键字，定义静态方法.
  static void prinntCount() {
    print("当前人数: " + _count.toString());
  }
}
```
